...
namespace Tp1Module6.Controllers
{
    public class ArmesController : Controller
    {
        ...
        {
            bool estAssociee = false;
            Arme arme = db.Armes.Find(id);
            List<int> ArmeAssociees = db.Samourais.Where(x => x.Arme != null).Select(x => x.Arme.Id).ToList();
            foreach (int armeAssociee in ArmeAssociees)
            {
                if (armeAssociee == id)
                {
                    estAssociee = true;
            ...
            if (estAssociee == true)
            {
                ModelState.AddModelError("", "Impossible de supprimer une arme utilisée par un samouraï !");
                return View(arme);
            }
